...
function getCountries() {
  return new Promise((resolve, reject) => {
    fetch(`${BASE_URL}/countries`)
      ...
      .catch(err => {
        reject(err);
        errorDiv.innerText = "impossible to retrieve the list of countries";
      });
  });
}
...
getStatistics("India").then(stats => {
  displayStatistics(stats);
});
  ...
  const countryStats = `
        <div class="row">
            ...
            <div class="col-sm-4">
                <div class="card">
                <div class="card-body">
                    <h5 class="card-title">Recover : ${stats.recovered.value}</h5>
                </div>
                </div>
            </div>
             <div class="col-sm-4">
        ...
        <div class="row">
            <div class="col-sm-12 mt-4" style="color:white;">Data updated on ${niceDate} </div>
        </div>
    `;
